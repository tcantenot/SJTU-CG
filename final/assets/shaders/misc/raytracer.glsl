...
Intersect trace(Ray ray) {
    ...
    spheres[0] = Sphere(2.0, vec3(-4.0, 3.0 + sin(uTime), 0), Material(vec3(1.0, 0.0, 0.2), 1.0, 0.001));
    spheres[1] = Sphere(3.0, vec3( 4.0 + cos(uTime), 3.0, 0), Material(vec3(0.0, 0.2, 1.0), 1.0, 0.0));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec2 uv    = fragCoord.xy / uResolution.xy - vec2(0.5);
