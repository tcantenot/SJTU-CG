...
vec3 noised( in vec2 x )
{
	...
	float a = texture(uTexture0,(p+vec2(0.5,0.5))/256.0,-100.0).x;
	float b = texture(uTexture0,(p+vec2(1.5,0.5))/256.0,-100.0).x;
	float c = texture(uTexture0,(p+vec2(0.5,1.5))/256.0,-100.0).x;
	float d = texture(uTexture0,(p+vec2(1.5,1.5))/256.0,-100.0).x;
...
{
    float d = 0.0;//50.0*texture( uTexture2, x*0.03/SC, 0.0 ).x;
    return d + 0.5*texture( uTexture2, x*2.0/SC, 0.0 ).x;
}
...
{
    float f = 0.0;
    f += 0.5000*texture( uTexture0, p/256.0 ).x; p = m2*p*2.02;
    f += 0.2500*texture( uTexture0, p/256.0 ).x; p = m2*p*2.03;
    f += 0.1250*texture( uTexture0, p/256.0 ).x; p = m2*p*2.01;
    f += 0.0625*texture( uTexture0, p/256.0 ).x;
    return f/0.9375;
}
...
vec3 render( in vec3 ro, in vec3 rd )
{
	...
	else
	{
        ...
        // rock
		float r = texture( uTexture0, (7.0/SC)*pos.xz/256.0 ).x;
        col = (r*0.25+0.75)*0.9*mix( vec3(0.08,0.05,0.03), vec3(0.10,0.09,0.08),
                                     texture(uTexture0,0.00007*vec2(pos.x,pos.y*48.0)/SC).x );
...
{
    vec2 xy = -1.0 + 2.0*fragCoord.xy/uResolution.xy;
	vec2 s = xy*vec2(uResolution.x/uResolution.y,1.0);

    float time = uTime*0.15 + 0.3 + 4.0*uMouse.x/uResolution.x;
