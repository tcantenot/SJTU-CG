...
float noise1( in vec3 x )
{
	...
	vec2 rg1 = texture( iChannel2, (uv+ vec2(0.5,0.5))/256.0, -100.0 ).yx;
	vec2 rg2 = texture( iChannel2, (uv+ vec2(1.5,0.5))/256.0, -100.0 ).yx;
	vec2 rg3 = texture( iChannel2, (uv+ vec2(0.5,1.5))/256.0, -100.0 ).yx;
	vec2 rg4 = texture( iChannel2, (uv+ vec2(1.5,1.5))/256.0, -100.0 ).yx;
...
vec4 texture2DGood( sampler2D sam, vec2 uv, float bias )
{
    ...
    vec2 iuv = floor(uv);
    vec2 f = fract(uv);
	vec4 rg1 = texture( sam, (iuv+ vec2(0.5,0.5))/1024.0, bias );
	vec4 rg2 = texture( sam, (iuv+ vec2(1.5,0.5))/1024.0, bias );
	vec4 rg3 = texture( sam, (iuv+ vec2(0.5,1.5))/1024.0, bias );
	vec4 rg4 = texture( sam, (iuv+ vec2(1.5,1.5))/1024.0, bias );
